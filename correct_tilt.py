...
def correction_homography(img_shape, pitch_deg, roll_deg, fov_h_deg=80.0):
    """
    Compute the homography H that maps the tilted image to a nadir-corrected image.

    The drone camera points straight down when level.  When the drone pitches or
    rolls, the image plane tilts away from horizontal.  We apply the inverse
    rotation (−pitch, −roll) through the camera model to produce a virtual nadir
    view.

        H = K · R_correction · K⁻¹

    Points in the original image are mapped to their positions in the corrected
    image by H·p (using homogeneous coordinates).
    """
    h, w = img_shape[:2]
    K    = _build_K(w, h, fov_h_deg)

    p = math.radians(-pitch_deg)   # negate: we undo the tilt
    r = math.radians(-roll_deg)

    # Pitch correction: rotate image around horizontal axis (X)
    Rx = np.array([[1, 0,            0           ],
                   [0, math.cos(p), -math.sin(p) ],
                   [0, math.sin(p),  math.cos(p) ]], dtype=np.float64)

    # Roll correction: rotate image around vertical axis (Y)
...
def correct_tilt(img, pitch_deg, roll_deg, fov_h_deg=80.0):
    """Apply tilt correction; return (corrected_img, H)."""
    h, w = img.shape[:2]
    H = correction_homography(img.shape, pitch_deg, roll_deg, fov_h_deg)
...
def process_one(img_path: Path, pitch_deg, roll_deg, fov_h_deg,
    ...
    if json_path.exists() and (p is None or r is None):
        with open(json_path) as f:
            att = json.load(f)
        if p is None: p = float(att.get('pitch_deg', 0.0))
        if r is None: r = float(att.get('roll_deg',  0.0))
        print(f'  Attitude from JSON: pitch={p:.1f}°  roll={r:.1f}°')
    ...
    corrected, _ = correct_tilt(img, p, r, fov_h_deg)

    out_name = (img_path.stem
                + f'_corrected_p{p:.0f}r{r:.0f}'
                + img_path.suffix)
...
def main():
    ...
    for img_str in args.images:
        ...
        print(f'\n{img_path.name}')
        process_one(img_path, args.pitch, args.roll, args.fov,
